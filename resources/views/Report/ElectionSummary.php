<?php
    ...
    foreach($votesTally as $position => $candidates){
        ...
        $pdf->line($startW-$extend,$startH-$rowH,$endW+$extend,$startH-$rowH);
        $fontSize = 15;
        $pdf->addTextWrap($startW, $startH+$pdf->C2P(0.3), $endW, $fontSize, "<b>".$position."</b>",'center');
        $startH-=$rowH;

        $fontSize = 13;
        $pdf->addTextWrap($startW+$pdf->C2P(0.3), $startH+$pdf->C2P(0.3), $pdf->C2P(13.8), $fontSize, "<b>CANDIDATES</b>",'left');
        $pdf->addTextWrap($startW+$pdf->C2P(14), $startH+$pdf->C2P(0.3),$pdf->C2P(4.1), $fontSize, "<b>VOTES</b>",'center');
        ...
        foreach($candidates as $name => $candidate){
            ...
            $fontSize = 13;
            $pdf->addTextWrap($startW+$pdf->C2P(0.3), $startH+$pdf->C2P(0.3), $pdf->C2P(13.8), $fontSize, strtoupper($name),'left');
            $votes = gettype($candidate) == "array" ? number_format(count($candidate)) : number_format($candidate);
            $pdf->addTextWrap($startW+$pdf->C2P(14), $startH+$pdf->C2P(0.3),$pdf->C2P(4.1), $fontSize, $votes,'center');
        }
        $startH-=$pdf->C2P(0.5);
    }
    $pdf->ezStream();
